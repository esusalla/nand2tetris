...
bool parse_and_write(std::vector<std::string>& filepaths, const std::string& outpath)
{
    ...
    for (const auto& filepath : filepaths) {
        ...
        while (parser.has_more_commands()) {
            ...
            } else if (command_type == CommandType::C_LABEL) {
                code_writer.write_label(parser.arg1());
            } else if (command_type == CommandType::C_GOTO) {
                code_writer.write_goto(parser.arg1());
            } else if (command_type == CommandType::C_IF) {
                code_writer.write_if(parser.arg1());
            } else if (command_type == CommandType::C_CALL) {
                code_writer.write_call(parser.arg1(), parser.arg2());
            } else if (command_type == CommandType::C_RETURN) {
                code_writer.write_return();
            } else if (command_type == CommandType::C_FUNCTION) {
